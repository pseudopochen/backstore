...
export default class ProductAddUpdate extends Component {
  ...
  loadData = (selectedOptions) => {
    ...
    const subCategory = this.getCategories(targetOption.value);

    // load options lazily
    setTimeout(() => {
      targetOption.loading = false;
      if (subCategory && subCategory.length > 0) {
        targetOption.children = this.initOptions(subCategory);
      } else {
  ...
  async componentDidMount() {
    ...
    if (isUpdate) {
      if (product.pCategoryID !== "0") {
        ...
        subCategory = this.getCategories(product.pCategoryID);

        const subOptions = this.initOptions(subCategory);
